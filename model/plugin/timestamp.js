...
module.exports = function timestampSchemaPlugin(schema, options) {
	...
	log.debug(`timestampSchemaPlugin(): schema=${inspect(schema)}, options=${inspect(options)}, this=${inspect(this)}`);
	...
	schema.post('validate', function(doc, next) {
		...
		// } else if (doc._ts.createdAt && doc.isNew) {
		// 	return next(new Error(`[model ${model.modelName}].post('validate')#timestampSchemaPlugin: doc._ts.createdAt && doc.isNew ${doc.isModified()?'':'!'}doc.isModified()`));
		// }
		var now = Date.now();
		if (doc.isNew) {
		...
		log.verbose(`[model ${model.modelName}].post('validate')#timestampSchemaPlugin: isNew=${doc.isNew} modifiedPaths=${doc.modifiedPaths().join(' ')}`);
		return next();
	});

	
	schema.method('markDeleted', function(timestamp = Date.now()) {
		if (this._ts.deletedAt) { throw new Error(`Doc being marked deletedAt=${timestamp} already has deletedAt=${this._ts.deletedAt}`); }
