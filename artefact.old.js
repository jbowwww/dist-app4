...
Artefact.prototype = {
	...
	async toDocuments() {
		...
		return await Promise.all(
			Array.from(this._docs.values()).map(async value => {
				const resolvedValue = (typeof value.exec === 'function' ? await value.exec() : await Promise.resolve(value)); // unnecessary? wouldn't Promise.all just internally Promise.resolve any immediate _docs.values() ?
	...
	async do(options = {}, ...pipelineFuncs /*= (a) => {}*/) {
		// try {
			...
			// try {
			let self = await this.toDocuments();
			self = Object.fromEntries(self.map(doc => [doc.modelName, doc]));
			log.verbose(` # self = ${inspect(self)}`);//.filter(doc => doc != null);
			await Promise.all(pipelineFuncs.map(async fn => {
				try {
					// log.verbose(` ! typeof self.toDocuments = ${typeof self.toDocuments}, self = ${inspect(self)}`);
					await this.setArtefact(await fn(self)); /*toObject*/
				} catch (e) {
			...
			log.verbose(`Artefact#${self._id}.do(${inspect(options)}):${pipelineFuncs.length} pipeline funcs returned a=${inspect(self)}`);
			if (options.save)
				await self.save();
			else
				log.verbose(`Artefact _id=${self._id} do func is not configured to save()`);
			return self;
		// } catch (e) {
